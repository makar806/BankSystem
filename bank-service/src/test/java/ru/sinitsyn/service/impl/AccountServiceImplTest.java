...
public class AccountServiceImplTest {
    ...
    @BeforeEach
    void setUp(){
        accountService = new AccountServiceImpl(accountDao, userDao, operationDao);
    }
    ...
    void deposit_shouldIncreaseBalanceAndSaveOperation() {
        ...
        operationService.deposit(1L, new BigDecimal("50"), "login1");
    ...
    void withdraw_shouldDecreaseBalanceAndSaveOperation() {
        ...
        operationService.withdraw(1L, new BigDecimal("70"), "login2");
    ...
    void withdraw_shouldThrowInsufficientFundsExceptionWhenBalanceIsNotEnough() {
        User user = new User("login3", "Ivan", 2000-01-01, HairColor.BLONDE, Gender.MALE);
        Account account = new Account(new BigDecimal("30"), user);

        when(accountDao.findById(1L)).thenReturn(account);

        assertThrows(InsufficientFundsException.class, () -> operationService.withdraw(1L, new BigDecimal("100"), "login3"));
    ...
    void transfer_shouldThrowInblablablaWhenBalanceIsNotEnough() {
        ...
        assertThrows(InsufficientFundsException.class, () -> transferService.transfer(1L,2L, new BigDecimal("100"),"login6"));
